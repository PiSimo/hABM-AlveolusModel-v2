...
#ifndef CORE_ANALYSER_INSITUMEASUREMENTS_H
#define CORE_ANALYSER_INSITUMEASUREMENTS_H
...
#include "core/analyser/pair_measurement.h"
...
public:
  // Class for providing the main functionality of taking measurements during a simulation run
    ...
    virtual void observeMeasurements(const SimulationTime &time);
    virtual void setSite(Site *site) { site_ = site; }
    virtual void writeToFiles(const std::string &output_dir) const;
...
protected:
    std::map<std::string, double> time_last_measurement_{};
    Site *site_{};
...
#endif /* CORE_ANALYSER_INSITUMEASUREMENTS_H */
